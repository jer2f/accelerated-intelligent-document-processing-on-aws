...
- **Extraction: advanced mode can stop sending the document schema three times.** Two of the three are the same JSON object — 2,595 of 5,692 schema tokens (46%) on the lending `Payslip` class — and `extraction.agentic.restate_schema_in_system_prompt: false` drops the duplicate. ⚠️ **Measured, it buys nothing:** no completeness or accuracy cost, and no benefit either. Every copy sits inside the prompt cache, so the dollar saving is roughly a tenth of the token count, and reclaiming the tokens does **not** make long documents split into fewer parts — the schema text is not counted when the pipeline decides how to split a document, so they come out of a safety margin that was already unused ([#775](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/775)). It stays **on by default** anyway: our corpus is not yours, and restating a schema in prose often improves adherence — so if you turn it off, judge it on **completeness**, because a token saving that loses list rows is a loss. **Configuration → Prompt Preview** counts the duplicated block — as a floor, since the real block the backend generates is larger — so the saving is visible before you switch it off ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).
