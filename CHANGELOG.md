...
- **Legacy `.doc` files now fail with an actionable message instead of processing as a blank page — and are no longer offered for upload.** `.doc` was accepted by the upload picker and routed by the OCR step, but the Word converter reads the modern OOXML container only (python-docx) and no pure-Python reader exists for the legacy binary format, so such a file converted to one empty page and the document completed classification and extraction with **no content** — the failure surfaced later as inexplicably empty results. The converter now detects the legacy container by its bytes (so a `.doc` renamed `.docx` is caught too) and fails the document, naming the remedy: re-save as `.docx`. The upload picker and its format label no longer list `.doc`, and the PII Anonymization hook reports the same actionable error rather than an internal one. Legacy `.xls` is unaffected and remains supported. (#829)
- **PII Anonymization: legacy `.xls` workbooks are redacted instead of failing the run.** The hook accepted `.xls` — the upload picker offers it and the hook routed it — but handed it to a redactor that reads workbooks with `openpyxl`, which cannot open the legacy binary format at all, so the run failed (closed, with no PII leak, because the shipped preset sets `onError: fail`). Such a workbook is now rewritten as `.xlsx` before redaction, which is the format the redacted copy was already written in. (#824)
- **A genuinely low-confidence wide table can no longer cost a document its status-tracking record.** Confidence alerts are stored inline in the tracking item and every section's alerts share that one item, so the list had to be bounded or the item could exceed DynamoDB's 409,600-byte limit — at which point the write fails and the run records no result at all. The duplicate-alert fix below removed the cause seen in production, but per-row alerts are legitimately distinct and cannot be collapsed, so a table with many low-confidence cells reached the same ceiling on its own (512 rows × 8 columns is about 4,096 alerts). The stored list is now capped per section and per document, keeping the **lowest-scoring** alerts and recording the true count alongside them; nothing is lost, because every field's confidence remains in the extraction result's `explainability_info`, which is what the Web UI's own count reads. (#821)
- **Documents with many low-confidence fields no longer lose their status-tracking record during batched assessment.** Each assessment batch re-checked the same non-list fields and re-reported every alert, so a large document could inflate its tracking record past DynamoDB's item ceiling — the write failed and the run showed no result at all. One alert per finding is now kept, the lowest-scoring copy. (#814)
- **A transient failure inside the extraction or assessment Lambda was never retried, while the same failure that ran into the Lambda timeout was.** Step Functions retries by the reported error *name*, and the extraction and assessment tasks listed only the Bedrock modeled codes — so a botocore read timeout, a dropped connection or a Strands wrapper around one failed the document outright on the default runtime, and code that failed fast was penalised relative to code that hung. The handlers now classify the failure (`idp_common.utils.transient_errors`, walking the cause chain) and re-raise a transient cause under one stable name, `TransientError`, which the five extraction/assessment task states (in-process extraction, shard plan, shard, merge, assessment) retry. The classifier judges an exception by its own error code first and follows only explicit `raise … from` links, so a retry loop's chained attempts cannot make a deterministic failure look transient. Deterministic failures keep their own names and are still not retried; `ShardExtractionStep`, which used to blanket-retry `States.TaskFailed`, now follows the same transient-only rule ([#787](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/787)).
- **Classification over-split a single multi-page document because a table's reprinted column headings looked like a new heading.** With `sectionSplitting: llm_determined` on a single-class config, the boundary decision called the second or third page of a three-page bank statement `start` in about half of the runs, so one statement became two or three sections — silently, since every fragment still extracted completely. The `<boundary-detection-rules>` block gains a table-continuation rule: a page of table rows whose column headings repeat at the top, and that carries no opening identity block, is a continuation page. Measured with the real OCR and classification services on the benchmark fixtures (Nova 2 Lite): the unpaginated statement went from 8/15 to 15/15 correct section counts, the two-documents and paginated fixtures stayed 5/5; a same-stack A/B on a deployed stack (5 repeats, the frozen v0.6.7 prompt as control) scored 1/5 → 5/5 on that statement with the other fixtures unchanged. Every committed copy of the prompt (the `ocr-benchmark` presets, the sample and notebook configs) carries the same block; a configuration profile saved on a running stack is an independent snapshot and keeps its stored prompt — reset it to the default or re-apply the block to pick the rule up. A reprinted *title and account header* on every page (the #750 shape, now a benchmark fixture `repeathdr_3pg`) is still not handled by the shared rules — its supported fix remains the per-class `BOUNDARY:` sentence ([#726](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/726)).
...
- **Test Studio file patterns now match nested folders, and treat every non-wildcard character literally ([#808](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/808)).** `folder/**/*.pdf` matched nothing nested: `*` could not cross `/` and `**` was not a construct. Literals were not escaped either, so a `.` matched any character and `(`, `+` or `[` in a document name mis-matched or errored, including in rule validation's section-file lookup. Patterns now follow ordinary glob rules — `**` any depth, `*` one folder level, `?` one character — and are case-insensitive, so `*.pdf` finds `.PDF`. The listing is narrowed to the folder the pattern names instead of paging the whole bucket; since S3 prefixes are exact-case, that folder path must be typed as it exists.

- **Simple extraction with `integrated` confidence no longer loses list rows: list-bearing classes are scored in a separate pass automatically.** Simple + `integrated` (1S-TopK) puts values and per-cell confidence for the whole section into one response, and on a class with a list field the model stopped emitting rows rather than erroring — at the shipped default extraction model, 1–10 of 100 rows across 4 of 4 repeats, 5–10 of 400, and an 800-row list absent entirely, every one reporting `COMPLETED` with scalar accuracy 1.000, cheapest and fastest in the grid because it was doing the least work (mean recall 0.294). A Simple-mode section whose class declares any list field now runs the plain extraction prompt and lets the standalone Assessment step score it, so it gets complete rows plus batched confidence. ⚠️ **This costs more than the single inference configured:** the separate pass scores long lists in sequential batches, and on the same benchmark grid the `separate` cell cost $0.608 and took 268 s per 100-row document against $0.247 and 35 s for `integrated` — about 2.5× the cost and 7.7× the wall-clock at the shipped default model (Claude Sonnet 5), for a result that is actually complete. The delta is model-dependent: at Claude Sonnet 4.6 the same cell was *cheaper* than the integrated call in the 2026-09-09 live pass ($0.131 vs $0.171 per document). `metadata.confidence_mode_effective: separate` and the **Processing Flow** record it per document — deliberately not as a Processing Issue, because the document-level issues flag is severity-blind and would badge every document for a routing decision; `idp-cli config validate` (and the SDK validate operation) warns and names the affected classes, and the web UI's Prompt Preview shows per class that the plain prompt is sent (the UI does not validate on save). Two per-class opt-outs keep a list-bearing class on 1S-TopK: its own `x-aws-idp-extraction-task-prompt` (the downgrade works by swapping the prompt), or the new `x-aws-idp-allow-integrated-lists: true` flag for a class whose lists you have verified come back complete — the shipped 1S-TopK reference config and its notebook set it on their `Invoice` class so they still demonstrate the single-call path. The flag is a free-form class key that older releases ignore, so a stored configuration keeps loading on rollback. Scalar-only classes keep the single-inference saving, and Advanced extraction is unchanged (sharding keeps each call small). Deliberately a per-section decision rather than a config rejection.

- **Extraction: an escalation re-extraction that returned *less* data could be kept as an improvement.** With `extraction.validation.fail_action: escalate`, the stronger model's result replaced the original if it was valid or had fewer validation errors in total. Both halves of that test could keep a result that destroys data. Per-row errors scale with the row count while a whole-field error is always one, so a 100-row list with one unreadable cell per row (100 errors) lost to the same list returned as `null` (1 error) — the rows were discarded and the audit trail recorded the error count improving from 100 to 1. And a *valid* result with 50 clean rows where the original had 100 was kept as a fix rather than recognised as a truncation. The gate now refuses any result that loses populated data — a list that had rows must not come back null, absent or shorter, and a non-null value must not come back null — and otherwise compares errors per field, so no field may get worse and at least one must get better. The decision and its reason are recorded in the validation metadata and shown in the **Processing Report**, so a rejected escalation is no longer indistinguishable from a kept-but-still-invalid one. Each escalated field is now accepted on its own merits, so one field the stronger model got wrong no longer discards the ones it got right. ⚠️ **This is stricter than before:** a re-extraction that is valid but carries *less* populated data — a shorter list, or a filled value returned as `null` — is now kept only when the original value was itself invalid (a retracted hallucination, a `maxItems`/`uniqueItems` fix); previously any valid result was kept. Simple-mode escalation, which merged the stronger model's answer unconditionally, now uses the same gate. The escalation seed is also no longer built from an all-null model, which would have told the stronger model it was resuming from a checkpoint and must not re-extract ([#791](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/791)).
- **Advanced extraction can now say "I could not read this cell" instead of inventing a value.** A `required` attribute was rendered as a non-nullable field in the model the extraction agent fills in, so an unreadable value had no representable answer — of `null`, an empty string, omitting the field and `0.0`, only **`0.0`** was accepted — and the retry loop then told the agent its answer had failed validation and asked it to fix it. For a numeric attribute that produced a **fabricated zero that is schema-valid, raises no error, and is indistinguishable from a real zero**; on one unreadable column it happened for all 100 rows while the section reported as valid. Simple mode was never affected, because it emits plain JSON and a `null` survives to be reported. Every required *scalar* in that model is now nullable while `required` itself is kept, so a `null` cell is accepted but an omitted key, a nulled list, an empty tool call or a misspelled key set still fails exactly as before. Required-ness is then enforced where it can be reported rather than fabricated: `extraction.validation` checks the result against the real schema, treats a null as absent, reports `'X' is a required property` exactly as simple mode does, and feeds the agent one self-correction round that now says to fill a cell only if it can actually read it and never to null a whole list. The per-shard agentic runtime gains that same self-correction round, which it never had. Cells the agent could not read are counted under `metadata.abstained_fields` — scalar cells only, a nulled list is still a defect — and shown in the **Processing Report**, regardless of whether validation is enabled. The per-shard self-correction is scoped to what a shard *can* fix (types, formats, enums): presence and row-count bounds are not enforced per shard, because a shard is told to leave out-of-shard fields null, and are checked once on the merged section. Two consequences to know: with `fail_action: escalate` an honest abstention now triggers a paid re-extraction where the old contract's fabricated value validated silently (the default `warn` is free); and the agent's restated schema grows by roughly 25 characters per required scalar, which sits in the cacheable prefix but invalidates existing prompt caches once on deploy ([#782](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/782)).
- **Forced tool use: a group attribute whose name contains a space came back from Claude Sonnet 5 as a JSON string, failing every section.** The class schema sent as a tool renamed *property* names to Bedrock's `^[a-zA-Z0-9_.-]{1,64}$` rule but left `$defs` definition names alone, since Bedrock never validated those — so a group called `Account Holder Address` produced a `$ref` pointer containing spaces, which Sonnet 5 does not resolve (Sonnet 4.6 did, which is why it went unseen). On the shipped `bank-statement-sample` class with forcing on, 0 of 19 sections validated. Definition names are now made wire-safe the same way, every local `$ref` (raw or percent-encoded) is rewritten to match, and each `$ref` node gains the definition's `type` as belt and braces. A group the model still returns as a serialized string is parsed **while its field names are restored**, so the stored result carries the authored names rather than the wire spellings — for any number of groups (the restore used to guess only when a class had exactly one definition, so the shipped lending package's three-group class kept wire spellings), and `metadata.forced_tool.renamed_definitions` counts the renames. Independently, value coercion now parses a string that *is* the JSON of the object or array the field expects — lossless only: `NaN`/`Infinity`, overflow and duplicate keys are refused — and records it as `json_parsed_from_string`; a field that also allows a string is left alone ([#783](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/783)).

- **The RBAC test harness no longer clears the rest of the Web UI app client.** `make api-test` temporarily enables an auth flow to mint tokens, and sent only that one property to `UpdateUserPoolClient` — which is a full replace, so it also cleared `CallbackURLs`, `LogoutURLs`, `AllowedOAuthFlows`, `SupportedIdentityProviders`, `ReadAttributes`, `WriteAttributes` and the token validities. The hosted-UI login and external IdP federation stopped working after every test run, and CloudFormation did not repair it: the client is unchanged in the template, so the next stack update skips it. It now reads the client, changes the one field, and sends the rest back untouched. ⚠️ **A stack that had `make api-test` run against it may still be carrying the damage** — the values are restored by re-deploying onto a fresh client, or by setting them back with `aws cognito-idp update-user-pool-client` to match `template.yaml`.

- **External IdP group mapping now requires the group claim to have come from the configured provider.** `ExternalIdPGroupMappingFunction` assigned Cognito groups from the `custom:idp_groups` user-pool attribute without establishing that the external IdP was what set it. It now honours the attribute only for a user linked to the provider named by `ExternalIdPName` — determined from the Cognito-managed `identities` attribute — and only on a fresh sign-in, not a token refresh; anything it cannot confirm leaves group membership unchanged and adds no token override. `UserPoolClient` also now declares an explicit `WriteAttributes` listing only the attributes the IdP maps, rather than relying on Cognito's default. ⚠️ **Applies only to deployments with an external IdP and `ExternalIdPGroupAttributeName` configured** — without both, the trigger is not created. **Configure your IdP to send the group attribute for every federated user**, including users in no group, so each sign-in carries an authoritative value. See [external IdP](docs/external-idp.md).

- **`getStepFunctionExecution` now checks the execution ARN it is given.** The resolver passed the caller-supplied `executionArn` to `DescribeExecution` and `GetExecutionHistory` as received, so it would return the input, output and step history of any execution its IAM role could reach — which spans a sibling deployment whose stack name extends this one's, since the grant is a `<stack-name>-*` prefix. It now requires the ARN to name this deployment's state machine before calling Step Functions, and applies the caller's `allowedConfigVersions` to the configuration version the execution ran under. Previously listed as GAP-01 in the API authorization expectations; `make api-test-static` now fails if either check is removed.

- **A transient Bedrock timeout could consume a whole Lambda invocation asleep.** Agentic extraction retried the agent with a backoff that doubled up to **1,800 seconds — twice the 900-second limit of the function it runs in** — so a single `Read timed out` could spend the entire invocation waiting and achieve nothing. That is why one cost run showed 2 of 5 documents aborting in advanced integrated mode. Any one backoff is now capped at 60 seconds and the total across attempts at 300, and the retry decorators additionally **shorten** any sleep that would not fit in the time left in the current invocation, so the remaining time goes to another attempt instead of to sleeping. A sleep is only ever shortened, never turned into a failure: running out of time still ends as a Lambda timeout, which Step Functions retries with completed shards skipped, whereas failing early would surface an error name the Extraction and Assessment states do not retry. The Extraction handlers publish the deadline from `context.get_remaining_time_in_millis()`; where it is unknown — local runs, tests, other functions — retry behaviour is unchanged ([#786](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/786)).
...
- **Log groups named after their Lambda function no longer invert the create order or leave never-expiring orphans behind.** 21 `AWS::Logs::LogGroup` resources — the pipeline-hooks dispatcher, plus the API, hook and UI-deployer functions across the seven feature-platform extensions that declare log groups, including the `feature-template` every new extension is cloned from — set `LogGroupName: !Sub '/aws/lambda/${SomeFunction}'`. That resolves the function's generated name, so the *log group* depended on the *function* and CloudFormation built the function first; anything invoking it in that window made Lambda auto-create `/aws/lambda/<fn>` itself and CloudFormation's `CREATE` then failed `ResourceAlreadyExists` — which bites custom-resource Lambdas hardest, since CloudFormation invokes those during the same stack operation. It also renamed the group whenever the function was replaced, and a straggler invocation logging after CloudFormation deleted the old group recreated it **with no retention policy**, because Lambda's auto-create sets none: on one long-lived stack that had accumulated **79 never-expiring orphan log groups holding 14.8 MiB**, against 17 groups actually carrying the retention the template asked for. Each is now `/${AWS::StackName}/lambda/<FunctionLogicalId>` with a matching `LoggingConfig` on the function, which both creates the group first and keeps the name stable across function replacement — the convention `patterns/unified`'s other 18 groups already used. ⚠️ **On an already-deployed stack this renames a live log group:** CloudFormation creates the new one and deletes the old, discarding that group's history (typically under 2 KB — the bulk of each function's logs is in the orphans, which are untouched). Two further consequences: a feature stack's log groups now sit under `/<host-stack>-feature-<id>/…`, which falls within the prefix the host stack's force-cleanup deletes, so tearing down a host can now remove a still-installed feature's live log groups; and any saved CloudWatch query, alarm or console bookmark pinned to an old `/aws/lambda/…` name needs repointing. **Pre-existing orphans are not cleaned up by this change** and can be deleted by hand; they are identifiable as groups under `/aws/lambda/<stack>-*` with `retentionInDays: null`. ([#818](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/818))
...
- **Extraction: advanced mode can stop sending the document schema three times.** Two of the three are the same JSON object — 2,595 of 5,692 schema tokens (46%) on the lending `Payslip` class — and `extraction.agentic.restate_schema_in_system_prompt: false` drops the duplicate. Every copy sits inside the prompt cache, so on a class whose prompt prefix clears the model's **minimum cacheable prefix** the dollar saving is roughly a tenth of the token count — but ⚠️ **a class under that minimum never caches at all** (silently: no error, no metric), and there the saving is worth its **full** face value. Two of the nine classes in the shipped `ocr-benchmark` preset are under it. See [prompt caching, measured](docs/benchmarking/prompt-caching.md). It stays **on by default** anyway: restating a schema in prose often improves adherence — so if you turn it off, judge it on **completeness**, because a token saving that loses list rows is a loss. **Configuration → Prompt Preview** counts the duplicated block — as a floor, since the real block the backend generates is larger — so the saving is visible before you switch it off ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).
